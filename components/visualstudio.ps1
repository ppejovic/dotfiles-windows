...
if ((Get-Command vswhere -ErrorAction SilentlyContinue) -ne $null) {
    $vsLatest = (vswhere -latest -format json) | ConvertFrom-Json
}

if ($vsLatest) {
    
    $env:DevEnvDir    =  Join-Path $vsLatest.installationPath "Common7\IDE\"
